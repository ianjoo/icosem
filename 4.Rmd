...
**Iconicity and visual languages in a multimodal language faculty** [(Note: Changed to virtual presentation)]{style="color:blue"}
...
Program (as of `r Sys.Date()`) [[(pdf)](IcoLL2026Program.pdf)]{style="color:blue"}
...
Program <- read_xlsx('IcoLL2026program.xlsx') %>% 
  ...
  mutate(Schedule = ifelse(grepl('Plenary talk',
                      ...
                      'Plenary talk',
                       Schedule)) %>% 
  mutate(Author = ifelse(Schedule == 'Plenary talk',
                         sub(".*\\(([^)]*)\\).*", "\\1", Title),
                         Author)) %>% 
  mutate(Title = ifelse(Schedule == 'Plenary talk',
                         gsub("\\s*\\([^)]*\\)", "", Title),
                         Title))    %>% 
  mutate(Title = 
           ifelse(Schedule == 'Opening remarks',
                  'Disentangling iconicity',
                  Title)) %>% 
  mutate(Author = 
           ifelse(Schedule == 'Opening remarks',
                  'Hinano Iida',
                  Author)) %>% 
  mutate(Schedule = 
           ifelse(Schedule == 'Registration (IB012)',
                  'Registration',
                  Schedule)) %>% 
  mutate(Room = 
           ifelse(Schedule == 'Registration',
                  'IB012',
                  Room))
...
[Book of abstracts](IcoLL2026Abstracts.pdf)
